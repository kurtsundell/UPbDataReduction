function varargout = Two_Sample_Compare_KDE(varargin)
% TWO_SAMPLE_COMPARE_KDE M-file for Two_Sample_Compare_KDE.fig
%      TWO_SAMPLE_COMPARE_KDE, by itself, creates a new TWO_SAMPLE_COMPARE_KDE or raises the existing
%      singleton*.
%
%      H = TWO_SAMPLE_COMPARE_KDE returns the handle to a new TWO_SAMPLE_COMPARE_KDE or the handle to
%      the existing singleton*.
%
%      TWO_SAMPLE_COMPARE_KDE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TWO_SAMPLE_COMPARE_KDE.M with the given input arguments.
%
%      TWO_SAMPLE_COMPARE_KDE('Property','Value',...) creates a new TWO_SAMPLE_COMPARE_KDE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Two_Sample_Compare_KDE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Two_Sample_Compare_KDE_OpeningFcn via varargin.
%
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Two_Sample_Compare_KDE_OpeningFcn, ...
                   'gui_OutputFcn',  @Two_Sample_Compare_KDE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Two_Sample_Compare_KDE is made visible.
function Two_Sample_Compare_KDE_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Two_Sample_Compare_KDE (see VARARGIN)
% Choose default command line output for Two_Sample_Compare_KDE
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% --- Outputs from this function are returned to the command line.
function varargout = Two_Sample_Compare_KDE_OutputFcn(hObject, eventdata, handles) 
...
data1 = xlsread(d1); %matrix of sample 1
...
xA = transpose(xA); %transpose data matrix
tin=linspace(1,length(xA),length(xA)); %create x linespace
A = data1(:,1);  %rename data
n = length(A); %number of data points
[bandwidth,kdeA,xmesh1,cdf]=kde(A,length(tin),a,b); %create kde matrix
kdeA=transpose(interp1(xmesh1, kdeA, tin)); %transpose kde matrix
...
data2 = xlsread(d2); %matrix of sample 2
...
xB = transpose(xB); %transpose data matrix
tin=linspace(1,length(xB),length(xB)); %create x linespace
B = data2(:,1); %rename data
n = length(B); %number of data points
[bandwidth,kdeB,xmesh2,cdf]=kde(B,length(tin),d,e); %create kde matrix
kdeB=transpose(interp1(xmesh2, kdeB, tin)); %calcuate and transpose kde matrix
...
xlswrite([path file], dat); %write Excel file
